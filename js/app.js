(function() {
	...
	var terrorWords = [
		'bomb', 'terrorism', 'bombing', 'taliban', 'Al Qaeda', 'attack', 'terror', 'islam', 'unjust', 'crime', 'hate crime', 'national threat', 'homeland security', 'shooting', 'riot', 'north korea', 'hazmat', 'radioactive', 'virus', 'national security'
	];
	...
	function processData(data) {
		...
		// }
		if (negativeWords.some(function(v) { return data.text.toLowerCase().indexOf(v) !== -1; })) {
		...
		} else if (terrorWords.some(function(v) { return data.text.toLowerCase().indexOf(v) !== -1; })) {
			displayData(data, terror);
		}
	}
